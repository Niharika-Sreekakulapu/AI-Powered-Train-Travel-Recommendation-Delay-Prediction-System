...
% Automated formatting: prefer newtx fonts & vector figures (trigger CI)
\IfFileExists{IEEEtran.cls}{%
	\documentclass[conference]{IEEEtran}
}{%
	% Use article class in draft diagnostic mode to make layout issues visible
	\documentclass[11pt,draft,onecolumn]{article}
\linespread{1.05} % slightly looser leading for readability
	...
	% Prefer Times-like text+math if available, otherwise fall back to lmodern
	\IfFileExists{newtxtext.sty}{%
	  \usepackage{newtxtext,newtxmath}
	  \PackageWarning{paper}{Using newtx (Times-like) text and math fonts for consistent typesetting}
	}{%
	  \usepackage{lmodern}
	}
...
\usepackage{microtype}
\usepackage{float}
\usepackage{cmap} % improves copy/paste and font mapping in PDFs
\usepackage{hyperref}
\usepackage{caption}
