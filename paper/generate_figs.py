...
try:
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTDIR, 'feature_importance.pdf'), bbox_inches='tight')
    plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTDIR, 'pred_vs_true_scatter.pdf'), bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(OUTDIR, 'error_coverage.pdf'), bbox_inches='tight')
plt.close()
...
tbl.set_fontsize(9)
plt.tight_layout()
plt.savefig(os.path.join(OUTDIR, 'dataset_stats.pdf'), bbox_inches='tight')
plt.close()
