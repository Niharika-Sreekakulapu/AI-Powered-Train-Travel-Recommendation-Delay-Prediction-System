...
\begin{itemize}
  \item Construct `route` as `source-destination` and scale numeric features such as distance.
  \item Encode categorical features (route, weather, season) using label encoders saved with model artifacts.
  \item Apply imputation pipelines for missing RailRadar features and flag imputed values (flags v4–v7 pipeline in `scripts/`).
