...
Because no experiment re‑runs were requested, we report the results present in the repository and documentation. The demo model reports the following metrics on the held‑out test split used in `train_v2_minimal.py` and `simple_train.py`:
\begin{table}[h]
\centering
...
Feature importance (reported by the saved RandomForest) ranks distance as the most important feature (≈52.2\%), followed by season and month. Figure~\ref{fig:featimp} shows the top features as extracted from the saved `model_v2.pkl` artifact.
